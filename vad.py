...
class VoiceActivityDetector():
    ...
    #timeLine从0.01~275.00
    #有27500个数据
    def detect_speech(self,timeLine):
        ...
        while (sample_start < (len(data) - sample_window)):
            ...
            #speech_ratio可在一定程度上反映概率
            speech_ratio = sum_voice_energy/sum_full_energy
            #print(sum_voice_energy,sum_full_energy)
            tmpResult=speech_ratio
            if(sum_full_energy==0):
                tmpResult=0
            else:
                #timeLine.append(speech_ratio)
                tmpResult=speech_ratio
            from math import e
            tmpResult=1/(1+e**(5*(0.5-tmpResult)))
            timeLine.append(tmpResult)
        ...
        print("10ms时间区间有几个？",count)
        #一共有27581个窗口，即27581个10ms
        #print("窗口数量:",len(detected_windows))
